...
class G29_1(GCodeCommand):  # was G29C[ircle]
    ...
    def execute(self, g):
        ...
        points_pr_circle = g.get_int_by_letter(
            "P", 8)          # Points pr circle
        ...
        theta = np.linspace(0, reverse * 2 * np.pi,
                            points_pr_circle, endpoint=False)
...
class G29_2(GCodeCommand):  # was G29S[quare]
    ...
    def execute(self, g):
        ...
        points = g.get_int_by_letter(
            "P", 16)                   # Points in total
        probe_start_height = g.get_float_by_letter(
            "S", 6.0)    # Probe starting point above bed
        probe_speed = g.get_float_by_letter("K", 3000)
        probe_offset_x = g.get_float_by_letter(
            "X", 0)          # Offset X from starting point
        probe_offset_y = g.get_float_by_letter(
            "Y", 0)          # Offset Y from starting point
        ...
        for i in range(len(probes)):
            if invert:
                gcodes += "    G30 P{} S F{}; Probe point {}\n".format(
                    len(probes) - i - 1, probe_speed, len(probes) - i - 1)
            else:
                gcodes += "    G30 P{} S F{}; Probe point {}\n".format(
                    i, probe_speed, i)
        gcodes += "    G31 ; Dock probe\n"
